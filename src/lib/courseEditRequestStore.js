...
function normalizeRequest(request = {}) {
  ...
  return {
    ...
    branchId: normalizeText(request.branchId),
    targetBranchId: normalizeText(request.targetBranchId || request.branchId),
    targetBranchEmail: normalizeText(request.targetBranchEmail).toLowerCase(),
    targetBranchName: normalizeText(request.targetBranchName),
...
function notifyBranchCourseEditUpdated(request) {
  ...
  return addNotification({
    ...
    message: changeSummary || `${request.facultyName || 'Faculty'} saved module and submodule changes for ${request.courseName || 'the course'}.`,
    actionLabel: 'Updated',
    targetSection: 'courses',
    branchId: request.targetBranchId || request.branchId,
    targetBranchId: request.targetBranchId || request.branchId,
...
export function recordCourseEditChange(requestId = '', changeSummary = '') {
  ...
  updateNotification(updatedRequest.sourceNotificationId || updatedRequest.id, {
    ...
    message: normalizeText(updatedRequest.changeSummary) || `${updatedRequest.facultyName || 'Faculty'} saved module and submodule changes for ${updatedRequest.courseName || 'the course'}.`,
  })
