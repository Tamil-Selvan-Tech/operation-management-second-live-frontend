...
export function normalizeFacultyRecord(record, fallback = {}) {
  ...
  const batchEntries = sourceBatchEntries.map((entry) => ({
    ...
    startTime: entry.startTime || entry.fromTime || '',
    startPeriod: entry.startPeriod || 'AM',
    endTime: entry.endTime || entry.toTime || '',
  ...
  return {
    ...
    batchEntries: Array.isArray(payload.batchEntries)
      ? payload.batchEntries.map((entry) => ({
          ...
          startPeriod: String(entry?.startPeriod || 'AM').trim().toUpperCase(),
          endTime: String(entry?.endTime || entry?.toTime || '').trim(),
          endPeriod: String(entry?.endPeriod || 'AM').trim().toUpperCase(),
          courseId: String(entry?.courseId ?? '').trim(),
