...
function normalizeBatchRow(row = {}) {
  return {
    ...
    weekType: normalizeText(row.weekType).toUpperCase(),
    mode: normalizeText(row.mode).toUpperCase(),
    courseStartDate: normalizeText(row.courseStartDate || row.startDate),
    courseEndDate: normalizeText(row.courseEndDate || row.endDate),
