...
import { FacultyWeekOffAdminRequests } from '../components/FacultyWeekOffAdminRequests'
...
export function InstituteLeavePage({ initialViewMode = 'institute' }) {
  ...
  const facultyPageCount = Math.max(1, Math.ceil(facultyRequests.length / facultyPageSize))
  const safeFacultyPage = Math.min(Math.max(1, facultyPage), facultyPageCount)
  const visibleFacultyRequests = facultyRequests.slice((safeFacultyPage - 1) * facultyPageSize, safeFacultyPage * facultyPageSize)
  useEffect(() => {
    setFacultyPage(current => Math.min(Math.max(1, current), facultyPageCount))
  }, [facultyPageCount])
  ...
  return <section className="institute-leave-page">
    ...
    {viewMode === 'faculty' || viewMode === 'weekoff' ? <>
      <div className="institute-leave-view-tabs" role="tablist" aria-label="Faculty leave management"><button type="button" className={viewMode === 'faculty' ? 'is-active' : ''} onClick={() => setViewMode('faculty')}>Faculty Leave Requests</button><button type="button" className={viewMode === 'weekoff' ? 'is-active' : ''} onClick={() => setViewMode('weekoff')}>Week-Off Requests</button></div>
      {viewMode === 'weekoff' ? <FacultyWeekOffAdminRequests /> : <section className="faculty-request-readonly-panel">
      <header className="institute-leave-header"><div><p>Review leave requests submitted by faculty in this branch.</p></div></header>
    ...
    ].map(({ label, key, icon: Icon, tone, note }) => <article key={key} className={`leave-stat-card tone-${tone}${data ? '' : ' is-loading'}`}><span className="leave-stat-icon"><Icon size={27} /></span><div className="leave-stat-copy"><span>{label}</span><strong>{data ? data.summary?.[key] ?? '—' : <span className="institute-skeleton institute-stat-skeleton-value" aria-hidden="true" />}</strong><small>{note}</small></div></article>)}</div>
