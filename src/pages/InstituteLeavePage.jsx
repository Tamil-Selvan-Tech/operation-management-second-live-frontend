...
export function InstituteLeavePage() {
  ...
  async function save(event) {
    ...
    try {
      await request(`/institute-leaves${cancel ? `/${cancel.id}` : form?.id ? `/${form.id}` : ''}`, {
        ...
        ...(cancel ? {} : { body: JSON.stringify({ leaveDate: form.leaveDate, leaveStartTime: form.leaveStartTime || null, leaveEndTime: form.leaveEndTime || null, reason: form.reason }) }),
      })
  ...
  return <section className="institute-leave-page">
    <header className="institute-leave-header"><div className="institute-leave-heading"><span className="institute-heading-icon"><CalendarDays size={34} /></span><div><p className="section-kicker">Management</p><h2>Institute Leave</h2><p>Manage institute-wide leaves and schedule changes</p></div></div>
      <button className="institute-primary" onClick={() => { setError(''); setFieldErrors({}); setForm({ leaveDate: formDate(data?.today), leaveStartTime: '', leaveEndTime: '', reason: '' }) }} disabled={!data}><Plus size={18} /> Declare Leave</button></header>
    ...
    <dialog ref={dialog} className={`institute-dialog ${cancel ? 'is-confirmation' : ''}`.trim()} onCancel={event => { event.preventDefault(); close() }}>
      ...
      {form ? <form onSubmit={save} noValidate><label>Leave Date *<input type="date" min={form.id ? undefined : data?.today} value={form.leaveDate} onChange={e => { setForm({ ...form, leaveDate: e.target.value }); setFieldErrors(current => ({ ...current, leaveDate: '' })) }} />{fieldErrors.leaveDate ? <small className="institute-field-error">{fieldErrors.leaveDate}</small> : null}</label><div className="institute-time-fields"><label>Start Time (optional)<input type="time" value={form.leaveStartTime || ''} onChange={e => setForm({ ...form, leaveStartTime: e.target.value })} /></label><label>End Time (optional)<input type="time" value={form.leaveEndTime || ''} onChange={e => setForm({ ...form, leaveEndTime: e.target.value })} /></label></div><small>Leave both times empty for a full-day leave.</small><label>Reason *<textarea maxLength={1000} value={form.reason} onChange={e => { setForm({ ...form, reason: e.target.value }); setFieldErrors(current => ({ ...current, reason: '' })) }} />{fieldErrors.reason ? <small className="institute-field-error">{fieldErrors.reason}</small> : null}</label><button className="institute-primary" disabled={busy}>{busy ? 'Saving…' : 'Save Leave'}</button></form> : null}
