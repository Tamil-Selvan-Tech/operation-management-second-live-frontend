...
export function SuperAdminOverallDashboard({ branches, userKey = 'super-admin' }) {
  ...
  return <div className="sa-overall-dashboard">
    ...
    <section className="sa-branch-performance" aria-labelledby="branch-performance-title">
      <div className="sa-branch-performance-header"><div><h2 id="branch-performance-title">Branch Performance</h2><p>Detailed performance across all active branches</p></div><div className="sa-branch-performance-controls"><label><span>Period:</span><select aria-label="Branch performance period" defaultValue="thisMonth"><option value="thisMonth">This Month</option></select></label><label><span>Sort:</span><select aria-label="Sort branch performance" value={branchPerformanceSort} onChange={(event) => setBranchPerformanceSort(event.target.value)}><option value="branchName">Branch Name</option><option value="totalStudents">Total Students</option><option value="admissions">This Month Admissions</option><option value="collection">Revenue / Collection</option><option value="due">Due</option><option value="outstanding">Outstanding</option><option value="attendance">Attendance</option></select></label></div></div>
      <div className="sa-branch-performance-table-wrap"><table className="sa-branch-performance-table"><colgroup><col className="sa-branch-col" /><col /><col /><col /><col /><col /><col /></colgroup><thead><tr><th>Branch</th><th>Total Students</th><th>This Month Admissions</th><th>Revenue</th><th>Due</th><th>Outstanding</th><th>Attendance</th></tr></thead><tbody>{isLoading ? <tr><td colSpan="7" className="sa-branch-performance-empty">Loading branch performance...</td></tr> : branchPerformanceRows.length ? branchPerformanceRows.map((branch) => <tr key={branch.branchId}><th scope="row" title={branch.branchName}><span className="sa-branch-name-line"><i className="sa-branch-status-dot" /> <strong>{branch.branchName}</strong></span><small>{branch.branchId}</small></th><td>{branch.totalStudents}</td><td>{branch.thisMonthAdmissions}</td><td className="sa-branch-performance-money">{formatOverviewCurrency(branch.collection)}</td><td className="sa-branch-performance-money"><span className="sa-performance-value">{formatOverviewCurrency(branch.due)}</span></td><td className="sa-branch-performance-money"><span className="sa-performance-value">{formatOverviewCurrency(branch.outstanding)}</span></td><td><span className={`sa-branch-attendance-badge ${branch.attendance == null ? 'is-empty' : branch.attendance >= 90 ? 'is-good' : branch.attendance >= 75 ? 'is-neutral' : 'is-warning'}`}>{branch.attendance == null ? '—' : `${branch.attendance}%`}</span></td></tr>) : <tr><td colSpan="7" className="sa-branch-performance-empty">No active branches available</td></tr>}</tbody></table></div><div className="sa-branch-performance-footer">Showing {branchPerformanceRows.length} active {branchPerformanceRows.length === 1 ? 'branch' : 'branches'}</div>
    </section>
