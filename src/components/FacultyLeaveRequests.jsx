...
const emptyForm = { leaveType: 'PLANNED', fromDate: '', toDate: '', durationType: 'FULL_DAY', halfDayStart: '', halfDayEnd: '', permissionHours: '1', permissionStart: '', reason: '' }
...
export function FacultyLeaveRequests() {
  ...
  async function submit(event) {
    ...
    if (form.durationType === 'PERMISSION' && (!form.permissionHours || !form.permissionStart)) {
      setError('Select permission duration and start time.')
      return
    }
    setSaving(true)
    try {
      await request('/faculty-leave-requests', { method: 'POST', body: JSON.stringify({ ...form, halfDayStart: form.durationType === 'HALF_DAY' ? form.halfDayStart : null, halfDayEnd: form.durationType === 'HALF_DAY' ? form.halfDayEnd : null, permissionHours: form.durationType === 'PERMISSION' ? Number(form.permissionHours) : null, permissionStart: form.durationType === 'PERMISSION' ? form.permissionStart : null }) })
      setForm(emptyForm)
    ...
    <div className="faculty-leave-layout">
      <form className="faculty-leave-form-card" onSubmit={submit} noValidate>
        ...
        <div className="faculty-leave-form-grid">
          ...
          <label>Duration *<select value={form.durationType} onChange={event => { update('durationType', event.target.value); if (event.target.value !== 'HALF_DAY') { update('halfDayStart', ''); update('halfDayEnd', '') }; if (event.target.value !== 'PERMISSION') { update('permissionStart', ''); update('permissionHours', '1') } }}><option value="FULL_DAY">Full Day</option><option value="HALF_DAY">Half Day</option><option value="PERMISSION">Permission Leave</option></select></label>
          ...
          {form.durationType === 'PERMISSION' ? <><label>Permission Duration *<select value={form.permissionHours} onChange={event => update('permissionHours', event.target.value)}><option value="1">1 Hour</option><option value="2">2 Hours</option></select></label><label>Permission Start Time *<input type="time" value={form.permissionStart} onChange={event => update('permissionStart', event.target.value)} /></label><p className="faculty-leave-time-help">Permission end time will be calculated automatically.</p></> : null}
